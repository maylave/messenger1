...
const renderHighlight = async () => {
  ...
  const codeString = Array.isArray(props.code) 
    ? props.code.join('\n') 
    : props.code
  ...
  highlightedHtml.value = String(codeString)
    .replace(/</g, '&lt;')
    .replace(/>/g, '&gt;')
}
}
...
<template>
  ...
  <pre
    class="text-xs font-mono whitespace-pre-wrap break-all leading-relaxed opacity-80 max-h-[200px] overflow-y-auto custom-scrollbar bg-transparent p-0 m-0"
