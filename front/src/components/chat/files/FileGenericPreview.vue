...
import CodePreviewModal from '@/components/chat/modal/CodePreviewModal.vue'
...
const isFullscreen = ref(false) // <-- Эта переменная теперь управляет модалкой через v-model
...
<template>
  <div class="w-full relative overflow-hidden">
    <!-- Верхняя часть: Текст и цвета -->
    <div v-if="textContent" class="p-4 pb-2 relative z-10">
      <!-- Используем previewLines для корректной работы кнопки свернуть/развернуть -->
      <CodePreview :code="previewLines.join('\n')" :language="resolvedLanguage" />
    </div>
    ...
    <div class="relative h-16 flex items-center justify-between px-4 mt-auto">
      ...
      <div class="flex items-center gap-3 relative z-10">
        <div
          class="p-2.5 w-10 h-10 flex items-center justify-center bg-frame-700/50 rounded-xl text-white border border-white/5"
        >
      ...
      <div class="flex items-center gap-2 relative z-10">
        ...
        <!-- Полноэкранный режим (Открывает модалку) -->
        <button
          v-if="textContent"
    ...
    <!-- 2. Подключаем модальное окно и привязываем v-model -->
    <CodePreviewModal
      v-if="textContent"
