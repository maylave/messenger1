...
<template>
  ...
  -->
  <div class="mt-2 flex items-center pl-1 select-none group justify-start w-fit max-w-full overflow-x-auto pb-2">
    
    <div class="flex items-center h-9 pr-4"> <!-- pr-4 дает отступ справа для кнопки -->
      <div
      ...
      >
        ...
        <!-- Кружок цвета -->
        <div
          class="w-7 h-7 rounded-full shadow-md border-2 border-white/20 
                 flex-shrink-0 transition-transform duration-300 
                 hover:scale-110 hover:border-white/50 hover:shadow-xl"
          :style="{ backgroundColor: color }"
        ></div>
        
        <!-- Свечение -->
        <div 
          class="absolute inset-0 rounded-full opacity-0 hover:opacity-100 
      ...
      <button
        ...
        class="relative z-[60] flex items-center justify-center w-8 h-8 rounded-full 
               bg-black/60 hover:bg-white/10 border-2 border-white/30 
               cursor-pointer transition-all duration-300 active:scale-95 
               flex-shrink-0 shadow-lg backdrop-blur-sm
